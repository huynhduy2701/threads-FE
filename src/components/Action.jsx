...
const Action = ({ post:post_ }) => {
  ...
  const handleLikeAndUnlike = async()=>{
    if (!user) {
      return showToast("Lỗi tương tác bài viết","Bạn phải đăng nhập để có thể tương tác bài viết này","error");
    }
    if (isLiking) {
      return
    }
      ...
      <Flex flexDirection={"column"}>
        <Flex gap={3} my={2} onClick={(e) => e.preventDefault()}>
          ...
          <svg
            ...
            onClick={
              () => setRepost(!repost)
              // Hành động khi nhấp vào nút "Đăng lại"

              // console.log("Đăng lại nội dung!");
              // Ở đây bạn có thể thêm hành động như gửi yêu cầu lên server hoặc cập nhật trạng thái.
            }
          >
